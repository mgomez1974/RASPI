...
using TMPro;
/*
cet script.08 marche avec Send_data_to_unity_momo_03.ino
*/
    ...
    public class script10 : MonoBehaviour
    {
		...
		void ReadSerial()
		{
			try
				...
				string pattern2 = @"[a-z]{2}:\W{0,1}\d{1,4}\.\d{1,4};";//https://docs.microsoft.com/en-us/dotnet/standard/base-types/regular-expression-language-quick-reference
					...
					TextMesh _Text04 = GameObject.Find("Text04").GetComponent<TextMesh>(); 
					TextMesh _Text05 = GameObject.Find("Text05").GetComponent<TextMesh>(); 
					TextMesh _Text06 = GameObject.Find("Text06").GetComponent<TextMesh>(); 
					TextMesh _Text07 = GameObject.Find("Text07").GetComponent<TextMesh>(); 
					TextMesh _Text08 = GameObject.Find("Text08").GetComponent<TextMesh>(); 
					TextMesh _Text09 = GameObject.Find("Text09").GetComponent<TextMesh>(); 
					...
					while (m2.Success) {
						...
						if (type.Contains("ga"))							
						{
							...
							_Text01.text="acceleration.x:" + a_sensor_c;
						...
						if (type.Contains("gb"))							
						{
							...
							_Text02.text="acceleration.y:" + a_sensor_c;
						...
						if (type.Contains("gc"))							
						{
							...
							_Text03.text="acceleration.z:" + a_sensor_c;

						}
						if (type.Contains("gd"))							
						{
							//gps[0] = (float)Convert.ToInt32(a_sensor_c);
							Debug.Log("gd:"+a_sensor_c);
							_Text04.text="magnetic.x:" + a_sensor_c;

						}
						if (type.Contains("ge"))							
						{
							//gps[0] = (float)Convert.ToInt32(a_sensor_c);
							Debug.Log("ge:"+a_sensor_c);
							_Text05.text="magnetic.y:" + a_sensor_c;

						}
						if (type.Contains("gf"))							
						{
							//gps[0] = (float)Convert.ToInt32(a_sensor_c);
							Debug.Log("gf:"+a_sensor_c);
							_Text06.text="magnetic.z:" + a_sensor_c;

						}
						if (type.Contains("gg"))							
						{
							//gps[0] = (float)Convert.ToInt32(a_sensor_c);
							Debug.Log("gg:"+a_sensor_c);
							_Text07.text="gyro.x:" + a_sensor_c;

						}
						if (type.Contains("gh"))							
						{
							//gps[0] = (float)Convert.ToInt32(a_sensor_c);
							Debug.Log("gh:"+a_sensor_c);
							_Text08.text="gyro.y:" + a_sensor_c;

						}
						if (type.Contains("gi"))							
						{
							//gps[0] = (float)Convert.ToInt32(a_sensor_c);
							Debug.Log("gi:"+a_sensor_c);
							_Text09.text="gyro.z:" + a_sensor_c;
