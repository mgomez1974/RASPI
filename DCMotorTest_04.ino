...
void TestLineSensor(int SensorX,int SensorLineX_answer, int SensorLineX_answer_old);
...
void loop() {
  ...
  TestLineSensor(1,SensorLine1_answer,SensorLine1_answer_old);
  
  SensorLine2_answer = digitalRead(SensorLine2_pin);
  TestLineSensor(2,SensorLine2_answer,SensorLine2_answer_old);
  
  SensorLine3_answer = digitalRead(SensorLine3_pin);
  TestLineSensor(3,SensorLine3_answer,SensorLine3_answer_old);
  
  SensorLine4_answer = digitalRead(SensorLine4_pin);
  TestLineSensor(4,SensorLine4_answer,SensorLine4_answer_old);
  
  SensorLine5_answer = digitalRead(SensorLine5_pin);
  TestLineSensor(5,SensorLine5_answer,SensorLine5_answer_old);

  /*if(millis()-lastMilli2 > 100)
  {
    Serial.println(SensorLine1_answer);
  }*/
...
void TestLineSensor(int SensorX,int SensorLineX_answer, int SensorLineX_answer_old)
{
  ...
  SensorLineX_answer = digitalRead(SensorLine1_pin);
    if (SensorLineX_answer != SensorLineX_answer_old)
    {
      if (SensorLineX_answer == 1 ) {Serial.print("S");Serial.print(SensorX);Serial.println("1");}
      else                          {Serial.print("S");Serial.print(SensorX);Serial.println("0");}
    }
}
